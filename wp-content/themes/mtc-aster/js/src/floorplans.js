(function(){
    ...
    var togglesContainer=$("#checkboxes"),
        ...
        unit_types=[],
        valuesArray=[],
        siteID = "3031826"; //@TODO this is hardcoded because it's not coming from the API
    ...
    function displayUnit(unit){
        ...
        if (checkExists(unitID)){
            ...
            if (typeof leaseOnlineURL != "undefined"){
                var leasingURL = checkExists(unitID) ? leaseOnlineURL + "?MoveInDate=" + moveInDate + "&UnitId=" + unitID + "&site_id=" + siteID + "&SearchUrl=" + document.location : "";
                unitTemplate.find(".availability a").attr("href",leasingURL);
                
                //for SEO purposes / Google Tag Manager
                unitTemplate.find(".availability a").attr({"data-unit-type":unit.FloorPlan.FloorPlanName,"data-unit-number":aptNum,"data-unit-type-category":unit.UnitDetails.Bedrooms + " Bed"});
            }
        }
        return unitTemplate;
    }
    ...
    function displayUnitTypeDetail(unitType){
        ...
        if (checkExists(name)) {
            ...
            unitDetail.find("img").attr("src",templateURL+"/images/floorplans/"+fileName+".png");
            $("#unit-pdf-link").attr("href",templateURL+"/pdf/"+fileName+".pdf");
        ...
        //for SEO purposes / Google Tag Manager
        bedString = bedString.replace(/<\/?[^>]+(>|$)/g, ""); //removing html tags
        $("#unit-pdf-link").attr({"data-unit-type":name,"data-unit-type-category":bedString});
    ...
    function displayUnitType(unitType){
        ...
        //for SEO purposes / Google Tag Manager
        unitRow.find('button').attr({"data-unit-type":name});
        unitRow.find('button').attr({"data-unit-type-category":bedbath});
    ...
    function getJSON(){
        $.ajax({
            cache:false,
            url:templateURL+"/JSON/realPageFloorPlans.json",
            dataType:"json",
        ...
        //get today's preloaded units (from realPage-units.json)--but if user searches for another date, pull in results on the fly from API
        $.ajax({
            cache:false,
            url:templateURL+"/JSON/realPageUnits.json",
            dataType:"json",
            success:function(data){
                units=data;
                console.log('units from JSON',units);
                if (floor_plans != null){
                    setupSearch();
                }
            }
    ...
    function getUnitsJSON(){
        ...
        $.ajax({
            ...
            success:function(data){
                units=data;
                console.log('units from PHP',units);
                if (floor_plans != null){
                    setupSearch();
                }
            }
    ...
    $(document).ready(function(){
        ...
        startDate = new Date($("#move-in-date").val());
        ...
        initActions();
        $("#move-in-date").datepicker({minDate:startDate});
    });
    $(window).on({
        load:function(){
            if (isDetailOpen){
